...
function PanelList(live, style, extras) {
    ...
    this.panel.port.on("configure", () => {
        this.panel.hide();
        emit(this, "configure");
    });

    this.panel.port.on("refresh", (channelId) => emit(this, "refresh", channelId));
    ...
    this.panel.port.on("open", (channelId) => {
        this.panel.hide();
        emit(this, "open", channelId);
    });
    ...
    this.panel.port.on("add", (type, login) =>  emit(this, "add", type, login));
    this.panel.port.on("explore", (type) => emit(this, "explore", type));
    this.panel.port.on("search", (type, q) => emit(this, "search", type, q));
    this.panel.port.on("ready", () => emit(this, "ready"));
...
PanelList.prototype.addExploreProviders = function(providers) {
    this.panel.port.emit("providers", providers);
};

PanelList.prototype.setFeatured = function(channels, type) {
    this.panel.port.emit("explore", channels, type);
};
