 ...
 *                                 Defaults to a function checking the status
 *                                 code for being bigger than 499.
 * @return {Promise} A promise resolving with the Add-on SDK Request response.
 */
QueueService.prototype.queueRequest = function(url, headers = {}, requeue = defaultRequeue) {
 ...
 * @argument {QueueService.LOW_PRIORITY|QueueService.HIGH_PRIORITY} priority.
 * @argument {updateRequestCallback} callback - Called whenever a request is
 *                                              done (for each provided URL).
 * @argument {Object.<string,string>} [headers={}] - A headers object to send
 *                                                   with each request.
 * @argument {requeue} [requeue] - Determinines if a request should be re-run.
 *                                 Defaults to a function checking if the
 *                                 response status is bigger than 499.
 */
