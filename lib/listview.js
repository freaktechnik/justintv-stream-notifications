 ...
 * @argument {module:channellist.ChannelList} chlist
...
function ListView(live, style, extras, chlist) {
    ...
    this.list.on("open", async(function*(channelId, what) {
        let channel;
        if(typeof channelId === "string") {
            ...
            channel = yield chlist.getChannel(channelId);
        }
    ...
    this.list.on("ready", () => {
        ...
        chlist.getChannelsByType(null).then((channels) => {
            if(channels.length > 0)
                this.addChannels(channels.map((c) => c.serialize()));
        });

        this.list.setProviders(serializeProviders(providers));
    });
}
/**
 ...
 * @argument {Array.<module:channeluser.Channel>} channels
 */
 ...
 * @argument {module:channeluser.Channel} channel
 */
