...
function ListView(live, style, extras, chlist) {
    ...
    this.list.on("openurl", (url) => tabs.open(url));
    ...
    this.list.on("explore", (type) => {
        providers[type].getFeaturedChannels()
            .then((channels) => this.list.setFeatured(channels, type), () => this.list.setFeatured([], type));
    });

    this.list.on("search", (type, q) => {
        providers[type].search(q).then((channels) => this.list.setFeatured(channels, type), () => this.list.setFeatured([], type));
    });
