...
exports.getStatusRequest = function(channel,ignoreNull,callback, offset, prev) {
    offset = offset || 0;
    return {
        url: baseURL+'/streams?channel='+channel+'&limit=100&offset='+offset,
        headers: header,
        onComplete: function (response) {
            console.log("channel updated");
            var s = exports.checkResponse(response);
            var json = response.json.streams;
            var l = s==1&&(json.length>0||(prev&&prev.length>0));
            console.log(channel+' is live: '+l);
            callback(s,l,addChannelsToList(prev, extractChannelsFromList(json)),type,ignoreNull);
        }
    };
};
