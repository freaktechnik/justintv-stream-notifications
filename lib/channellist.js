...
Channel.prototype.addRichInfo=function(channelRspObj) {
        ...
        */
        ...
        if(config.getPreference('panel.advancedStyle')) {
            ...
            this.style.hasBgImage = !!channelRspObj[config.getPreference("justintv.backgroundImageName")];
            if(this.style.hasBgImage) {
                this.style.bgImage = channelRspObj[config.getPreference("justintv.backgroundImageName")];
            }
        }
};
...
ChannelList.prototype.addChannel = function(channel,source,rich) {
    ...
    if(!exists&&(config.getPreference("panel.advancedStyle")||!rich)) {
    ...
    else if(!config.getPreference("panel.advancedStyle")&&rich&&!this.channels[name]) {
        this.channels.length++;
        this.channels[name] = new Channel(Channel.FULL,source);
    }
...
UpdateQueue.prototype.addRequest = function(requestArgs,persistent,priorized) {
    ...
    this.queue.push(mix(requestArgs,{'persist':persistent,'id':++this.lastID}));
    
    if(priorized) {
        this.getRequest(this.queue.length-1);
        this._skip++;
    }
...
// @override
UpdateQueue.prototype.getRequestBatch = function(batchSize) {
    batchSize = batchSize-this._skip || this.queue.length-this._skip;
    if(batchSize>this.queue.length) batchSize = this.queue.length-this._skip;
