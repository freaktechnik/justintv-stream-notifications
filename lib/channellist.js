...
Channel.prototype.addRichInfo=function(unifiedRspObj) {
        ...
        */
        ...
        if(config.getPreference('panel.advancedStyle')) {
            ...
            this.thumbnail = unifiedRspObj.thumbnail;
    ...
    for(var user in this.users[source.channelType]) {
        if(!exactArraySearch(userNames,user,false)) {
            if(this.users[source.channelType][user].sources.onlySourceSet(source)) {
                this.removeUser(user,source.channelType,source);
            }
...
ChannelList.prototype.removeUser = function(user,type,source) {
    var channelType = this.channels[type];

    // add source metainfo, as it might be missing from setUsers
    source.name = user;
    for(var channel in channelType) {
        if(channelType[channel].sources.isSourceSet(source)) {
            if(channelType[channel].sources.onlySourceSet(source)) {
                this.removeChannel(channel, type, true);
            }
            else {
                this.channels[type][channel].sources.usernames.splice(user,1);
            }
        }
    }
...
Sources.prototype.isSourceSet = function(source) {
    if(source.type==Source.TYPE_USERNAME) {
        for(var username in this.usernames) {
            if(this.usernames[username]==source.name)
                return true;
        }
        return false;
    }
