...
UpdateQueue.prototype.addRequest = function(arg1,persistent,priorized) {
    ...
    if(!this.requestQueued(arg1)) {
        if(arg1 instanceof Channel) {
            ...
            return this.queue.push({'request':channelProvider.getRequestOptions(arg1.login,channelProvider.statusRequest,true),'persist':true,'priorize':false})-1;
        }
        else {
            return this.queue.push({'request':arg1,'persist':persistent,'priorize':priorized})-1;
        }
    }
};
...
UpdateQueue.prototype.getRequestByIndex = function(index) {
    ...
    if(this.queue[index].persist) {
        this.queue.push({'request':this.queue.splice(index,1)[0],'persist':true,'priorize':false});
    }
    else {
...
UpdateQueue.prototype.getRequestIndex = function(query) {
    if(query instanceof Channel) {
        ...
        for(var r in this.queue) {
            if(this.queue[r].request === request) {
                return r;
            }
        }
    }
        ...
        for(var r in this.queue) {
            if(Request(this.queue[r].request) === query) {
                return r;
            }
        }
    }
