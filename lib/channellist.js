...
Channel.prototype.type = "justintv";
...
Channel.prototype.getFirstSourceObject = function(options) {
    ...
    return new Source(s,s==Source.TYPE_USERNAME?this.sources.usernames[0]:this.login,options);
};
...
ChannelList.prototype.addChannel = function(channel,source,rich) {
    ...
    if(!exists&&!rich) {
        ...
        emit(this, "completechannelinfo", name, source, "justintv");
    }
    else if(!exists) {
        this.channels.length++;
...
// @override
UpdateQueue.prototype.addRequest = function(request,persistent,priorized) {
    persistent = persistent || false;
    priorized = priorized&&!persistent || false;
    if(!this.requestQueued(request)) {
        if(request instanceof Channel) {
            var channelProvider = require("./"+request.type);
            this.queue.push({'request':channelProvider.getRequest(request.login,channelProvider.statusRequest,true),'persist':true,'priorize':false});
        }
        else {
            this.queue.push({'request':request,'persist':persistent,'priorize':priorized});
        }
    }
};
...
UpdateQueue.prototype.getRequestByIndex = function(index) {
    var request = extend({},this.queue[index].request);
    request.get();
    if(this.queue[index].persist) {
        this.queue.push({'request':this.queue.splice(index,1)[0],'persist':true});
    }
    else {
...
UpdateQueue.prototype.requestQueued = function(query) {
    if(query instanceof Channel) {
        var channelProvider = require("./"+query.type);
        var request = channelProvider.getRequest(query.login,channelProvider.statusRequest,true);
        for(var r in this.queue) {
            if(this.queue[r] === request) {
                return true;
            }
        }
    }
    ...
    else if(query instanceof require("sdk/request").Request) {
        for(var r in this.queue) {
            if(this.queue[r].request === query) {
                return true;
            }
        }
    }
