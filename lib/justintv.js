...
// this is secret
var oauthKey = prefs.justintv_apiKey;
...
exports.checkResponse = function(response) {
    ...
    if(response!=null) {
        ...
        if(response.status==200) {
            ...
            var r = !prefs.justintv_ignoreRatelimit&&json.hasOwnProperty("rate_limited")&&json.rate_limited&&!json.message.match(/^199\.9\.2(4[89]?|5[1-5]?)?\.[0-2][1-5]?[1-9]?/);
            if(!e&&!r) {
    ...
    return getRequestObject(channel,function(achannel,aignoreNull,acallback) {
        var liveStatusRequest = Request({
            ...
            onComplete: function (response) {
                ...
                acallback(s,l,extractChannelsFromList(json,'channel'),type,aignoreNull);
            }
        });
...
exports.getUnifiedChannelInfo = function(channelRspObj) {
    ...
    uniC.bgImage = channelRspObj[prefs.justintv_backgroundImageName];
    uniC.full = true;
...
function extractChannelsFromList(response,subproperty) {
    if(response.length>0) {
        var channelNames = [];
        response.forEach(function(channel) {
            if(channel.hasOwnProperty(subproperty))
                channelNames.push(exports.getBasicUniChannelInfo(channel[subproperty]));
            else
