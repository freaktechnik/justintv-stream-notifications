...
Notifier.prototype = {
    ...
    sendNotification(channel) {
        ...
        if(this.showNotifications && !channel.url.some((url) => url === tabs.activeTab.url)) {
            let title = null;
            if(channel.live.state == LiveState.LIVE && this.onlineNotifications && !this.channelTitles.has(channel.id)) {
            ...
            else if(channel.live.state > LiveState.LIVE && this.nonliveNotifications && this.channelTitles.get(channel.id) == channel.title && this._channelStateChanged(channel)) {
