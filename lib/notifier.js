...
export default class Notifier {
    ...
    sendNotification(channel) {
        ...
        if(this.showNotifications && !channel.url.some((url) => url === tabs.activeTab.url)) {
            let title = null;
            if(channel.live.isLive(LiveState.TOWARD_OFFLINE) && this.onlineNotifications && this._channelStateChanged(channel)) {
                title = _("onlineNotification", channel.toString());
            }
            else if(channel.live.isLive(this._getLiveInterpretation()) && (channel.live.state === LiveState.LIVE || (this.nonliveNotifications && !this._channelStateChanged(channel))) && this.titleNotifications && this.channelTitles.get(channel.id) != channel.title) {
                title = _("updateNotification", channel.toString());
            }
            else if(!channel.live.isLive(this._getLiveInterpretation()) && this.offlineNotifications && this.channelTitles.has(channel.id)) {
                title = _("offlineNotification", channel.toString());
            }
            else if(channel.live.state > LiveState.LIVE && this.nonliveNotifications && this._channelStateChanged(channel)) {
