...
exports.attachChannelList = function() {
    ...
    data.on('completechannelinfo',function(name,source,type) {
        exports.queue(type,new Source(source.type,name),false,require("./"+type).getChannelDetails,completeChannel,false,true);
    });
...
exports.addUserFavorites = function() {
    ...
    if(prefs.usernameList!='') {
        ...
        for(var username of usernames) {
           exports.queue("justintv",new Source(Source.TYPE_USERNAME,username,{'page':0}),false,require("./justintv").getUserFavorites,processUserFavorties,false,true);
         ...
         passwords.search({
            ...
            onComplete: function(credentials) {
                credentials.forEach(function(credential) {
                    exports.queue("justintv",new Source(Source.TYPE_CREDENTIALS,credential.username,{'page':0}),false,require("./justintv").getUserFavorites,processUserFavorties,false,true);                    
                });
            }
         });
         passwords.search({
...
function completeChannel(status,response,source,ignoreNull) {
    if(status==1)  {
        ...
        exports.queue("justintv",source,ignoreNull,require("./justintv").getChannelDetails,completeChannel,false,true);
    }
}
    ...
    if(status==1) {
        ...
        if(exactArraySearch(data.users,source.name,false)||source.type==Source.TYPE_CREDENTIALS) {
            ...
            if(response.length==100) {
                source.page++;
                exports.queue("justintv",source,ignoreNull,require("./justintv").getUserFavorites,processUserFavorties,false,true);
            }
        }
    }
    else if(!ignoreNull&&status==2)
