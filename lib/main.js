...
exports.addChannel = function(channel,source) {
	if(!this.channelExists(channel.login)) {
		...
		prototype.image[0] = channel.image_url_tiny;
        prototype.image[1] = channel.image_url_medium;
		prototype.title = "";
...
exports.onlySourceSet = function(sourceName,channel) {
    ...
    for(var source in data.channels[channel].sources) {
        if(source!=sourcePropertyName&&data.channels[channel].sources[source]&&typeof data.channels[channel].sources[source]=='boolean')
            ...
            for(var item of data.channels[channel].sources[source]) {
                if(item!=sourceName) {
                    return false;
                }
            }
...
exports.checkStatus = function(results,channel) {
    try {
        ...
        else if(data.channels[channel].title!=result[0].title&&result.length>0&&data.channels[channel].isLive) {
            data.channels[channel].title = result[0].title;
            data.channels[channel].image[0] = result[0].channel.image_url_tiny;
            data.channels[channel].image[1] = result[0].channel.image_url_medium;
