        ...
        livePanel.port.on("loaded",function() {
            ...
            for(var channel in data.channels) {
                    ...
                    if(channelObj.status == Channel.QUEUED) {
                        exports.queue(channelObj.type,channelObj.getFirstSourceObject(),false,require("./"+channelObj.type).getChannelDetails,completeChannel,false,true);
                    }
                    else {
                        livePanel.port.emit("add",channelObj);
                        exports.queue(channelObj.type,channel,true,require("./"+channelObj.type).statusRequest,exports.checkStatus,true,false);
            ...
            queue.autoFetch(1000*prefs.updateInterval,.5,8);
            exports.attach();
        });
    }
...
exports.attachChannelList = function() {
    ...
    data.on('completechannelinfo',function(name,source) {
        exports.queue(source.channelType,new Source(source.type,name,source.channelType),false,require("./"+source.channelType).getChannelDetails,completeChannel,false,true);
    });
    ...
    data.on('channeldeleted',function(channelObj) {
        ...
        if(channelObj.status!=Channel.QUEUED) {
            ...
            exports.unqueue(channelObj);
        }
        else {
            exports.unqueue(require("./"+channelObj.type).getRequestObject(channelObj.getFirstSourceObject(),require("./"+channelObj.type).getChannelDetails,false,completeChannel));
        }
    });
...
exports.addUserFavorites = function() {
    ...
    if(prefs.usernameList!='') {
        ...
        for(var username of usernames) {
           exports.queue("justintv",new Source(Source.TYPE_USERNAME,username,"justintv",{'page':0}),false,require("./justintv").getUserFavorites,processUserFavorties,false,true);
         ...
         passwords.search({
            ...
            onComplete: function(credentials) {
                credentials.forEach(function(credential) {
                    exports.queue("justintv",new Source(Source.TYPE_CREDENTIALS,credential.username,"justintv",{'page':0}),false,require("./justintv").getUserFavorites,processUserFavorties,false,true);                    
                });
            }
         });
         passwords.search({
...
function completeChannel(status,response,source,ignoreNull) {
    if(status==1)  {
        ...
        exports.queue(source.channelType,source,ignoreNull,require("./"+source.channelType).getChannelDetails,completeChannel,false,true);
    }
}
    ...
    if(status==1) {
        ...
        if(exactArraySearch(data.users,source.name,false)||source.type==Source.TYPE_CREDENTIALS) {
            ...
            if(response.length==100) {
                source.page++;
                exports.queue(source.channelType,source,ignoreNull,require("./"+source.channelType).getUserFavorites,processUserFavorties,false,true);
            }
        }
    }
    else if(!ignoreNull&&status==2)
...
exports.queue = function(type,channel,ignoreNull,completeFunction,callback,persist,priorize) {
    if(ignoreNull) {
        ...
        queue.addRequest(require("./"+type).getRequestObject(channel,completeFunction,ignoreNull,callback),persist,priorize);
    }
};
...
exports.checkStatus = function(status,result,channel,ignoreNull) {
    if(status==1) {
        ...
        this.queue(data.channels[channel].type,ignoreNull,channel,require("./"+data.channels[channel].type).statusRequest,exports.checkStatus,true,false);
    }
};
