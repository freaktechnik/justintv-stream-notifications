    ...
    if(status==1) {
        ...
        if(data.userExists(source.name,source.channelType)&&channelNames) {
            ...
            if(channelNames.length==100) {
                source.page++;
                data.users[source.channelType][source.name].requestID = queue.addRequest(providers[source.channelType].getUserFavorites(source,ignoreNull,processUserFavorites,channelNames),false,true);
            }
            else {
...
exports.checkStatus = function(requestStatus,liveStatus,uniChObj,type,ignoreNull) {
    if(requestStatus==1) {
        if(Array.isArray(uniChObj)||!uniChObj) {
            if(liveStatus) {
                uniChObj.forEach(function(uniCh) { 
                    checkOnlineChannel(uniCh,type);
                });
            }
        ...
        queue.addRequest(providers[type].getStatusRequest(login,ignoreNull,exports.checkStatus,uniChObj.length,uniChObj),false,false);
    }
};
