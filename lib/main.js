...
var { ChannelsManager } = require('./channels-manager');
...
exports.main = function() {
    ...
    var panelWidth = prefs.panel_minWidth;
    livePanel = require("sdk/panel").Panel({
      width: panelWidth+12,
      contentURL: self.data.url("live-panel.html"),
      contentScriptOptions: {'minWidth':panelWidth,'advancedStyling':prefs.panel_advancedStyle,'backgroundImage':prefs.panel_channelBackground,'showTitle':prefs.panel_alwaysShowTitle}
    });
    ...
    // reset data if simplestorage is full
        ...
        livePanel.port.on("loaded",function() {
            ...
            ChannelsManager.updateFavorites();
            
            // update the channel states, so possibly no-longer-live channels get hidden.
            queue.autoFetch(1000*prefs.updateInterval,1/prefs.queue_ratio,prefs.queue_maxRequestBatchSize);
            exports.attach();
        });
    }
};
...
exports.init = function() {
    ...
    queue.autoFetch(1000*prefs.updateInterval,1/prefs.queue.ratio,prefs.queue_maxRequestBatchSize);
...
exports.attach = function() {
    ...
    simplePrefs.on('updateInterval',function() {
        queue.changeInterval(1000*prefs.updateInterval,1/prefs.queue_ratio,prefs.queue_maxRequestBatchSize);
    });
    simplePrefs.on('manageChannels',function() {
        ChannelsManager.open(data, providers, queue.addRequest);
    });
...
exports.attachChannelList = function() {
    ...
    data.on('channeloffline',function(channelObj) {
        ...
        if(!data.liveStatus()) {
            if(prefs.panelAccess == 1 && panelButton.icon == liveIcons) {
                panelButton.icon = offlineIcon;
    ...
    data.on('channeldeleted',function(channelObj) {
        ...
        // update live status
        if(!data.liveStatus() && actionButton.icon["16"] == self.data.url("panel_icon.png")) {
            actionButton.icon["16"] = self.data.url("panel_icon_empty.png");
            actionButton.label = _("live_panel_label_offline");
        }
    });
    data.on('channellive',function(channelObj) {
        ...
        if(prefs.panelAccess == 1 && panelButton.icon == offlineIcons) {
            panelButton.image = liveIcons;
...
function processUserFavorites(status,channelNames,type,user,ignoreNull) {
    if(status==1) {
        console.log("user favorites valid");
        if(data.userExists(user, type)&&channelNames) {
...
exports.forceRefresh = function() {
    queue.getRequestBatch(prefs.queue_maxRequestBatchSize);
};
...
exports.checkStatus = function(requestStatus,liveStatus,uniChObj,type,ignoreNull) {
    if(requestStatus==1) {
        if(Array.isArray(uniChObj)||!uniChObj) {
            if(liveStatus) {
                for(var uniCh in%annel(uniChObj[uniCh],type);
                }
            }
