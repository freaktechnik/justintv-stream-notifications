...
exports.main = function() {
    this.addChannelProvider('justintv');
    this.addChannelProvider('twitch');
...
exports.attachChannelList = function() {
    ...
    data.on('completechannelinfo',function(name,source) {
        data.channels[name].requestID = exports.queue(providers[source.channelType].getChannelDetails(new Source(source.type,name,source.channelType),false,completeChannel),false,true);
    });
    ...
    if(prefs.usernameList!='') {
        ...
        for(var username of usernames) {
            ...
            if(data.users[username].sources.onlySourceSet(s))
                data.users[username].requestID = exports.queue(providers[type].getUserFavorites(new Source(Source.TYPE_USERNAME,username,type,{'page':0}),false,processUserFavorites),false,true);
        }
    }
    ...
    for(var channel in data.channels) {
        for(var username in data.channels[channel].sources.usernames) {
            if(!data.userExists(data.channels[channel].sources.usernames[username])||data.users.length==0) {
                if(data.channels[channel].sources.onlySourceSet(new Source(Source.TYPE_USERNAME,username,type))||data.users.length==0) {
            ...
            onComplete: function(credentials) {
                credentials.forEach(function(credential) {
                    ...
                    if(data.addUser(credential.username,new Source(Source.TYPE_CREDENTAILS,credential.username,"justintv")))
                        data.users[credential.username].requestID = exports.queue(providers["justintv"].getUserFavorites(new Source(Source.TYPE_CREDENTIALS,credential.username,"justintv",{'page':0}),false,processUserFavorites),false,true);    
                });
            }
         });*/
         passwords.search({
            ...
            onComplete: function(credentials) {
                ...
                credentials.forEach(function(credential) {
                    if(data.addUser(credential.username,new Source(Source.TYPE_CREDENTAILS,credential.username,"twitch")))
                        data.users[credential.username].requestID = exports.queue(providers["twitch"].getUserFavorites(new Source(Source.TYPE_CREDENTIALS,credential.username,"twitch",{'page':0}),false,processUserFavorites),false,true); 
                });
            }
         });
    }
...
function completeChannel(status,uniChObj,source,ignoreNull) {
    if(status==1)  {
        ...
        data.channels[source.name].requestID = exports.queue(providers[source.channelType].getChannelDetails(source,ignoreNull,completeChannel),false,true);
    }
    else if(status==0 {
        // remove channel if the response was "doesn't exist"
        data.removeChannel(source.name,true);
    }
}
    ...
    if(status==1) {
        ...
        if(data.userExists(source.name)&&channelNames) {
            ...
            if(channelNames.length==100) {
                source.page++;
                data.users[source.name].requestID = exports.queue(providers[source.channelType].getUserFavorites(source,ignoreNull,processUserFavorites),false,true);
            }
        }
    }
    else if(!ignoreNull&&status==2)
...
exports.checkStatus = function(status,result,channel,ignoreNull) {
    if(status==1) {
        ...
        data.channels[channel].requestID = this.queue(providers[data.channels[channel].type].getStatusRequest(channel,ignoreNull,exports.checkStatus),true,false);
    }
};
...
function getArchiveUrl(channel,index) {
    var channelObj = data.channels[channel];
    return URL(providers[channelObj.type].archiveURL.replace("%u",channelObj.url[index].toString()));
}
...
exports.addChannelProvider = function(name) {
    ...
    // do needed steps for channellist if necessary
