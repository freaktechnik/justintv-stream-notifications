    ...
    if(prefs.usernameList!='') {
        ...
        for(var username in usersArr) {
            if(usersArr[username].sources.onlySourceSet(s))
                usersArr[username].requestID = queue.addRequest(providers[type].getUserFavorites(new Source(Source.TYPE_USERNAME,username,type,{'page':0}),false,processUserFavorites),true,true,10);
        }
    }
};
    ...
    passwords.search({
        ...
        onComplete: function(credentials) {
            credentials.forEach(function(credential) {
                ...
                if(credential.username.length > 0 && data.addUser(credential.username,new Source(Source.TYPE_CREDENTAILS,credential.username,provider)))
