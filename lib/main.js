...
exports.main = function() {
    ...
    livePanel.port.on("log",function(msg) {
        console.log(msg);
    });
};
...
exports.setChannels = function(source,channelNames,rich) {
    ...
    if(channelNames.length>0&&channelNames[0]!='') {
        ...
        var prototype, name,channelObj;
        for each(var channel in channelNames) {
            name = rich?channel.login:channel;
            channelObj = new Channel('queued',name,source);
            data.channels[name] = channelObj;
            if(!rich)
...
exports.getChannelDetails = function(source) {
    var detailRequest = Request({
        ...
        onComplete: function (response) {
            if(!response.json.error&&data.channels[source.channel].status=='queued')
...
function Channel(status,channel,source) {
    if(status=='full') {
		this.addRichInfo(channel);
    }
    this.setSource(source);
}

Channel.prototype = {
    'addRichInfo':function(channelRspObj) {
        this.status='full';
    ...
    'setSource':function(source) {
        if(source.type=='username')
            this.sources.usernames.push(source.name);
        else
            this.sources[source.type+'Set'] = true;
    },
    'isLive':false,
    'mute':false,
    'title':'',
    ...
    'url':[],
    'names':[],
    'sources':{'userSet':false,'credentialsSet':false,'usernames':[]},
    'image':[],
    'status':'queued'};