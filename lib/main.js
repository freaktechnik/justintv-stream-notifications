...
exports.checkStatus = function(status,liveStatus,uniChObj,type,ignoreNull) {
    if(status==1) {
        try {
            ...
            if(liveStatus) {
                if(!channelObj.live) {
                    ...
                    if(!channelObj.mute&&prefs.titleChangeNotification)
                        this.sendNotification(channelObj.title,channelObj.image[1],uniChObj.login,type,true);
                }
            }
            else if(!liveStatus&&data.channels[type][uniChObj.login].live) {
