...
exports.main = function() {
    ...
    // reset data if simplestorage is full
        ...
        livePanel.port.on("loaded",function() {
            ...
            var channelObj, channelArr, count;
...
exports.attachChannelList = function() {
    ...
    data.on('channeloffline',function(channelObj) {
        ...
        if(!channelObj.mute&&prefs.offlineNotification)
				exports.sendNotification(channelObj.login,channelObj.type,false);
    ...
    data.on('channellive',function(channelObj) {
        if(!channelObj.mute&&prefs.onlineNotification)
            exports.sendNotification(channelObj.login,channelObj.type,true);
            ...
            onComplete: function(credentials) {
                credentials.forEach(function(credential) {
                    if(data.addUser(credential.username,new Source(Source.TYPE_CREDENTAILS,credential.username,"justintv")))
                        data.users["justintv"][credential.username].requestID = exports.queue(providers["justintv"].getUserFavorites(new Source(Source.TYPE_CREDENTIALS,credential.username,"justintv",{'page':0}),false,processUserFavorites),true,true,10);
                });
            }
         });
         passwords.search({
            ...
            onComplete: function(credentials) {
                credentials.forEach(function(credential) {
                    if(data.addUser(credential.username,new Source(Source.TYPE_CREDENTAILS,credential.username,"twitch")))
                        data.users["twitch"][credential.username].requestID = exports.queue(providers["twitch"].getUserFavorites(new Source(Source.TYPE_CREDENTIALS,credential.username,"twitch",{'page':0}),false,processUserFavorites),true,true,10);
                });
            }
         });
    }
            ...
            if(liveStatus) {
                if(!channelObj.live) {
                    ...
                    if(!channelObj.mute&&prefs.titleChangeNotification)
                        this.sendNotification(uniChObj.login,type,true);
                }
            }
            else if(!liveStatus&&channelObj.live) {
...
exports.sendNotification = function(channel,type,live) {
    try {
        var channelObj = data.channels[type][channel];
        Notifications.notify({
            title: (live ? _("onlineNotification",channelObj.toString()) : _("offlineNotification",channelObj.toString())),
