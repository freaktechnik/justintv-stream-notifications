...
/** @lends module:channel/manager.ChannelsManager.prototype */
{
    ...
    onChannelAdded(channelObj) {
        this.loading = false;
        // This leaks for all networks with username != login
        this.cancelingValues.delete("channel"+channelObj.type+channelObj.login);
        if(this.worker)
    ...
    onUserAdded(user) {
        this.loading = false;
        // This leaks for al providers where username != login
        this.cancelingValues.delete("user"+user.type+user.login);
        if(this.worker)
    ...
    onError(name, type, itemType) {
        this.loading = false;
        this.cancelingValues.delete(itemType+type+name);
        if(this.worker) {
            if(name)
