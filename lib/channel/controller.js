...
export default class ChannelController extends Disposable {
     ...
     * @return {Promise}
     * @resolves {Array.<module:channel/core.Channel>|module:channel/core.Channel}
     */
     ...
     * @argument {string} type
     * @return {Promise}
     * @resolves {Array} An array with the added user and an array of added
     *                   channels.
     * @rejects Gets rejected if the provider doesn't support favorites.
     */
    ...
    autoAddUsers(provider) {
        if(!provider || !(provider in providers)) {
            ...
            return Promise.all(providers[provider].authURL.map(
