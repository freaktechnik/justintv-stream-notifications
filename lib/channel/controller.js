...
const searchCredentials = (url) => new Promise((resolve, reject) => {
    passwords.search({
        url,
        onComplete: resolve,
        onError: reject
...
const filterExistingFavs = (user, channels) => channels.filter((ch) => !user.favorites.includes(ch.login));
...
export default class ChannelController extends Disposable {
     ...
     * @return {Array.<module:channel/core.Channel>|module:channel/core.Channel}
     */
     ...
     * @argument {string} type
     * @return {module:channel/core.User} An array with the added user and an array of added
     *                 channels.
    ...
    async _updateUser(user) {
        ...
        const [ finalUser, newChans ] = await Promise.all([
     ...
     * @argument {number|string} id - ID or login
     * @argument {string} [type] - Type if not an ID is given
     */
