...
/** @lends module:channel/controller.ChannelController.prototype */
{
    ...
    initialize(options) {
        ...
        let managerError = () => this._manager.loading = false;
        this._manager = new ChannelsManager({
            onAddchannel: (name, type) => this.addChannel(name, type).catch(managerError),
            onRemovechannel: (channelId) => this.removeChannel(channelId),
            onUpdatechannel: (channelId) => this.updateChannel(channelId).catch(managerError),
            onAdduser: (username, type) => this.addUser(username, type).catch(managerError),
            onRemoveuser: (userId) => this.removeUser(userId),
            onUpdatefavorites: (userId) => this.updateUser(userId).catch(managerError),
            onAutoadd: () => this.autoAddUsers().catch(managerError),
            onGetdata: () => {
                Promise.all([
                ...
                ]).then(managerError, managerError);
