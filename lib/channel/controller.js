...
/** @lends module:channel/controller.ChannelController.prototype */
{
    ...
    setup(options) {
        ...
        let managerError = (name, type) => {
            if(name && type in providers)
                this._manager.onError(name, providers[type].name);
            else
        ...
        this._manager = new ChannelsManager({
            ...
            onAdduser: (username, type) => this.addUser(username, type)
                            .catch(() => managerError(username, type)),
