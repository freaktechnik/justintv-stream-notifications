 ...
 * @type {Promise?}
 */
ChannelList.prototype._openingDB = null;
...
ChannelList.prototype.openDB = function(name, dontTry = false) {
    ...
    this._openingDB = promise;
    // Clear it once the promise is done.
    promise.then(() => this._openingDB = null, () => this._openingDB = null);
    ...
    req.onsuccess = () => {
        if(req.result) {
            ...
            if("favorites" in req.result) {
                ...
                resolve(deserialize(req.result, true));
            }
        }
        else
            reject();
        ...
        var req = store.index("typename").openCursor().onsuccess = (event) => {
            ...
            if(cursor) {
                if(!needsCleanup && "favorites" in cursor.value)
                    needsCleanup = true;
                retchans.push(deserialize(cursor.value, true));
                cursor.continue();
            }
            else {
                if(needsCleanup) {
