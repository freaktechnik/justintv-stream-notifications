...
exports.ToolbarButton = function ToolbarButton(options) {
  ...
  var delegate = {
    onTrack: function (window) {
      ...
      if(options.panel) {
        options.panel.on("hide",function _onPanelHide() {
            tbb.checked=false;
      ...
      tbb.addEventListener("command", function() {
        ...
        if (options.panel) {
          if(!tbb.checked&&tbb.checkState!=2) {
            var anchor = options.image ? doc.getAnonymousElementByAttribute(tbb,"class","toolbarbutton-icon"):tbb;
            options.panel.show(anchor);
            tbb.checked = true;
          }
          else
            tbb.checkState=0;
