...
exports.ToolbarButton = function ToolbarButton(options) {
  ...
  var delegate = {
    onTrack: function (window) {
      ...
      if(options.panel) {
        tbb.setAttribute("type","checkbox");
        options.panel.on("hide",function TB__onHide() {
            if(!suppress)
                tbb.removeAttribute("checked");
        });
        tbb.addEventListener("mouseover", function() {
            suppress = true;
        }, true);
        tbb.addEventListener("mouseleave", function() {
            suppress = false;
        }, true);
      }
  ...
  return {
    ...
    set panel(value) {
        if(options.panel)
            options.panel.removeListener("hide",TB__onHide);
            
        options.panel = value;
        if(options.panel===null&&tbb.type=="checkbox")
