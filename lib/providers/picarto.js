...
const Picarto = newClass({
    ...
    getChannelDetails: function(channelname) {
        return this._qs.queueRequest(baseURL + "/channel_img/" + channelname.toLowerCase() + "/dsdefault.jpg", headers, requeue)
            .then((resp) => {
    ...
    updateRequest: function(channels, callback) {
        ...
        this._qs.queueUpdateRequest(urls, headers, this._qs.HIGH_PRIORITY, requeue, function(page, url) {
    ...
    updateChannel: function(channelname) {
        ...
        return this._qs.queueRequest(baseURL + "/live/channel.php?watch=" + channelname, headers, requeue).then((page) => {
