...
const Picarto = newClass({
    ...
    getChannelDetails: function(channelname) {
        return this._qs.queueRequest(baseURL + "/user_data/usrimg/" + channelname.toLowerCase() + "/dsdefault.jpg", headers, requeue)
            .then((resp) => {
                if(resp.status < 400)
    ...
    updateRequest: function(channels) {
        ...
        this._qs.queueUpdateRequest(urls, headers, this._qs.HIGH_PRIORITY, requeue, (page, url) => {
