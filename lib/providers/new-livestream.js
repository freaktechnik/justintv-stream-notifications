...
const NewLivestream = newClass({
    ...
    _getChannelStatus: async(function*(json, channel) {
       // Checks if there are any upcoming or past events and if yes, if one is currently being broadcast.
        let event = (Array.isArray(json.upcoming_events) && json.upcoming_events.data.find((event) => event.broadcast_id != -1)) ||
            (Array.isArray(json.past_events) && json.past_events.data.find((event) => event.broadcast_id != -1));
