...
const Ustream = newClass({
    ...
    getChannelDetails: async(function*(channelname) {
        let data = yield this._qs.queueRequest("http://ustream.tv/"+channelname, headers, requeue);

        if(data.status == 200) {
            var channelId = data.text.match(/<meta name="ustream:channel_id" content="([0-9]+)">/)[1];
            let response = yield this._qs.queueRequest(baseURL+"channels/"+channelId+".json", headers, requeue);
