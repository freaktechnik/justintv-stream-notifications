...
let { Task: { spawn } } = require("resource://gre/modules/Task.jsm");
...
const Twitch = {
    ...
    updateChannel: function(channelname, callback, ignoreHosted) {
        return qs.queueRequest(baseURL+'/streams/'+channelname, headers, requeue).then((data) => {
            console.info("twitch.updateChannel.requestCallback");
            if(data.json && data.json.stream) {
                return this.getChannelDetails(channelname, function(channel) {
                ...
                return this.getChannelDetails(channelname, function(channel) {
                    if(!ignoreHosted) {
                        getHostedChannel(channel).then(callback);
                    }
                    else {
                        channel.live = false;
                        if(callback) callback(channel);
...
function getHostedChannels(channels, liveChans) {
    if(!prefs.twitch_showHosting) {
        ...
        var getHostingInfo = function(channelIds) {
            var hostIds = channelIds.join(",");
            var existingChans = Array.isArray(liveChans) ? channels.concat(liveChans) : channels;

            return qs.queueRequest("http://tmi.twitch.tv/hosts?include_logins=1&host="+hostIds, {}, requeue).then(function(data) {
                if(data.json && data.json.hosts.length) {
                    ...
                    return all(data.json.hosts.map(function(hosting) {
                        ...
                        if(hosting.target_login &&
                            ...
                            return Twitch.updateChannel(hosting.target_login).then(function(hostedChannel) {
        ...
        return spawn(function*() {
            ...
            return getHostingInfo(channelIds);
