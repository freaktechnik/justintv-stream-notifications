...
function getHostedChannel(channel, callback) {
    qs.queueRequest("http://chatdepot.twitch.tv/rooms/"+channel.login+"/host_target", {}, requeue, function(data) {
        if(data.json && data.json.host_target) {
            Twitch.updateChannel(data.json.host_target, function(hostedChannel) {
