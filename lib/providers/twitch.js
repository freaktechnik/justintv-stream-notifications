...
function getHostedChannel(channel, callback) {
    qs.queueRequest(baseURL+"/channels/"+channel.login, headers, requeue, function(resp) {
        if(resp.json && resp.json._id) {
            //TODO request multiple host targets at once, since we can do comma separated ids.
            qs.queueRequest("http://tmi.twitch.tv/hosts?include_logins=1&host="+resp.json._id, {}, requeue, function(data) {
                if(data.json && data.json.hosts.length && data.json.hosts[0].target_login) {
                    Twitch.updateChannel(data.json.hosts[0].target_login, function(hostedChannel) {
                        channel.uname = _("provider_twitch_hosted", channel.uname.split(" ")[0], hostedChannel.uname);
                        channel.live = hostedChannel.live;
                        channel.title = hostedChannel.title;
                        channel.thumbnail = hostedChannel.thumbnail;
                        channel.viewers = hostedChannel.viewers;
                        channel.category = hostedChannel.category;
                        callback(channel);
                    }, true);
                }
                else {
