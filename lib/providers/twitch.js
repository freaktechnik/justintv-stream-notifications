...
const Twitch = newClass({
    ...
    updateFavsRequest: function(users) {
        ...
        let updateCbk = (data) => {
            if(data.json && !data.json.error) {
                ...
                if(data.json.follows.length == itemsPerPage) {
                    ...
                    emit(this, "newchannels", ret[username].filter((chan) => !user.favorites.some((name) => name === chan.login)));
        ...
        this._qs.queueUpdateRequest(urls, headers, this._qs.LOW_PRIORITY, requeue, updateCbk);
    },
        ...
        new PaginationHelper({
            ...
            request: (url, callback, initial) => {
                if(initial)
                    ...
                    this._qs.queueRequest(url, headers, requeue, callback);
            },
