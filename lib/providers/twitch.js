...
const Twitch = newClass({
    ...
    updateFavsRequest: function(users, userCallback, channelCallback) {
        //TODO paginationhelperize?
        var urls = users.map(function(user) { return baseURL+'/users/'+user.login+'/follows/channels?limit='+itemsPerPage; }),
            ret  = [];
        let qR = this._qs.queueRequest;
        this._qs.queueUpdateRequest(urls, headers, this._qs.LOW_PRIORITY, requeue, function updateCbk(data) {
            if(data.json && !data.json.error) {
                ret = ret.concat(data.json.follows.map(function(channel) {
                    return getChannelFromJSON(channel.channel);
                }));
                if(data.json.follows.length == itemsPerPage) {
                    qR(data.json._links.next, headers, requeue).then(updateCbk);
                }
                else {
    ...
    updateRequest: function(channels, callback) {
        //TODO can this be paginationhelpered?
        var channelsString = channels.reduce(function(prev, curr) { return prev+(prev.length?',':'')+curr.login; },""), ret = [];
        let qR = this._qs.queueRequest;
        this._qs.queueUpdateRequest([baseURL+'/streams?channel='+channelsString+'&limit='+itemsPerPage+'&offset=0'], headers, this._qs.HIGH_PRIORITY, requeue, function updateCbk(data) {
            if(data.json && !data.json.error) {
                var cho;
                ...
                if(data.json.streams.length == itemsPerPage) {
                    ...
                    if(ret.length != channels.length) {
                        ...
                        this._getHostedChannels(offlineChans, Array.slice(ret)).then(callback);
                    }
                    ret.length = 0;
                }
            }
    ...
    _getHostedChannels: function(channels, liveChans) {
        if(!prefs.twitch_showHosting) {
            ...
            //TODO make sure the this inside the generator is set correctly.
