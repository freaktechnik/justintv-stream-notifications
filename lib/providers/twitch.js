        ...
        if(prefs.twitch_showHosting) {
            ...
            if(data.json && "hosts" in data.json && data.json.hosts.length) {
                ...
                return Promise.all(data.json.hosts.map((hosting) => {
                    ...
                    if(chan === undefined) {
                        //TODO try to get the original channel and its ID
                        return this.updateChannel(hosting.host_login, true);
                    }
                    else if(hosting.target_login &&
