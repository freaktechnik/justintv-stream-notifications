...
const Twitch = newClass({
    ...
    updateRequest: function(channels) {
        ...
        new PaginationHelper({
            ...
            onComplete: (data) => {
                const liveChans = data.filter((cho) => cho.live);
                if(liveChans.length) {
                ...
                if(liveChans.length != channels.length) {
                    ...
                    const playlistChans = data.filter((cho) => !cho.live);
    ...
    updateChannels: async(function*(channels) {
        ...
        const liveChans = ret.filter((cho) => cho.live);
