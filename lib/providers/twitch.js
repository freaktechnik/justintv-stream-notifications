...
const Twitch = newClass({
    ...
    updateRequest: function(channels) {
        ...
        new PaginationHelper({
            ...
            getItems: (data) => {
                if(data.json && "streams" in data.json) {
                    ...
                    return streams.map((obj) => {
                        ...
                        cho.live = true;
                        if(obj.is_playlist) {
                            cho.state = new LiveState(LiveState.REBROADCAST);
                        }
            ...
            onComplete: (data) => {
                if(data.length) {
                    Promise.all(data.map((channel) => {
                        if(channel.state.state == LiveState.REBROADCAST)
                            return this._getActivePlaylistTitle(channel).then((title) => {
                                channel.title = title;
                                return channel;
                            });
                        else
            ...
            ret = yield Promise.all(liveChannels.map((obj) => {
                ...
                if(logins.includes(cho.login)) {
                    ...
                    return Promise.all(channels.map((c) => this._getChannelId(c))).then((ids) => {
                        ...
                        if(obj.is_playlist) {
                            return this._getActivePlaylistTitle(cho).then((tile) => {
                                cho.title = title;
                                return cho;
                            }, () => cho);
                        }
                    ...
                    if(hosting.target_login &&
                       existingChans.every((ch) => ch.login !== hosting.target_login)) {
                        ...
                        return this.updateChannel(hosting.target_login, true).then((hostedChannel) => {
                            ...
                            chan.state = new LiveState(LiveState.REDIRECT);

                            if(!chan.live) {
                                chan.state.disable();
                            }
                            else {
                                chan.state.alternateUsername = hostedChannel.uname;
                                chan.state.alternateURL = hostedChannel.url[0];
                            }
