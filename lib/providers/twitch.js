...
class Twitch extends GenericProvider {
    ...
    async getUserFavorites(username) {
        ...
        if(data.json && !data.json.error) {
            const channels = await promisedPaginationHelper({
                ...
                user = new User(data.json.name, this._type);
    ...
    async _getActivePlaylistInfo(channel) {
        const id = await this._getChannelId(channel),
            playlist = await this._qs.queueRequest("https://api.twitch.tv/api/playlists/channels/" + id);

        if(playlist.json && playlist.json.enabled && playlist.json.active && playlist.json.playhead) {
            const playhead = playlist.json.playhead,
                vod = await this._qs.queueRequest(baseURL + "/videos/v" + playhead.vods[playhead.active_vod_index].id);
            if(vod.json) {
                return {
                    title: vod.json.title,
                    game: vod.json.game,
                    thumbnail: vod.json.preview
                };
            }
            else {
                throw "VOD not found";
