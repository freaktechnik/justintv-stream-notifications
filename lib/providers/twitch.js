...
class Twitch extends GenericProvider {
    ...
    async getUserFavorites(username) {
        ...
        if(data.json && !data.json.error) {
            ...
            const user = new User(data.json.name, this._type);
    ...
    updateFavsRequest(users) {
        ...
        this._qs.queueUpdateRequest(urls, this._qs.LOW_PRIORITY, (data) => {
            if(data.json && !data.json.error) {
                const user = users.find(user => user.login == data.json.name);
                user.uname = data.json.display_name;
                user.image = getImageObj(data.json.logo?data.json.logo:defaultAvatar);
    ...
    async updateChannels(channels) {
        ...
        const channelsString = logins.join(",");
        let liveChannels = await promisedPaginationHelper({
        ...
        if(liveChans.length != channels.length) {
            const playlistChans = await Promise.all(ret.filter((cho) => !cho.live.isLive(LiveState.TOWARD_OFFLINE)).map((cho) => {
                return this._getActivePlaylistInfo(cho).then((meta) => {
                    cho.title = meta.title;
                    cho.category = meta.game;
                    cho.thumbnail = meta.thumbnail;f
                    return cho;
                }, () => cho);
            }));
    ...
    async _getHostedChannels(channels, liveChans) {
        if(prefs.twitch_showHosting) {
            ...
            let data = await this._qs.queueRequest("https://tmi.twitch.tv/hosts?"+querystring.stringify({
                include_logins: 1,
    ...
    async _getActivePlaylistInfo(channel) {
        ...
        const playlist = await this._qs.queueRequest("https://api.twitch.tv/api/playlists/channels/"+id);

        if(playlist.json && playlist.json.enabled && playlist.json.active && playlist.json.playhead) {
            const playhead = playlist.json.playhead;
            const vod = await this._qs.queueRequest(baseURL + "/videos/v" + playhead.vods[playhead.active_vod_index].id);
            if(vod.json)
                return { title: vod.json.title, game: vod.json.game, thumbnail: vod.json.preview };
            else
                throw "VOD not found";
        }
        else {
