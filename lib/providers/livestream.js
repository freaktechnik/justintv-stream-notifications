...
const getLivestreamChannelInfo = function(username, callback) {
    qs.queueRequest(getChannelAPIUrl(username.toLowerCase())+"info.json", headers, requeue).then(function(data) {
        if(data.json && data.json.channel) {
            ...
            qs.queueRequest(getChannelAPIUrl(ch.login)+"latestclips.json?maxresults=1", headers, requeue).then(function(response) {
