...
const MLG = newClass({
    ...
    _getGame: async(function*(id) {
        ...
        if(!game) {
            let data = yield this._qs.queueRequest(gameURL, headers, requeue);
    ...
    getChannelDetails: async(function*(channelname) {
        let data = yield this._qs.queueRequest(infoURL+infoArgs, headers, requeue);
    ...
    updateRequest: function(channels, callback) {
        //TODO this in callback
        let jsonToChannel = this._getChannelFromJSON;
        this._qs.queueUpdateRequest([baseURL+"all"], headers, this._qs.HIGH_PRIORITY, requeue, async(function*(data) {
            let info = yield this._qs.queueRequest(infoURL+infoArgs, headers, requeue);
            ...
            if(data.json && data.json.status_code == 200 && info.json && info.json.status_code == 200) {
                ...
                chans = yield all(chans.map((status) => {
                    return jsonToChannel(info.json.data.items.find((ch) => ch.id == status.channel_id)).then((channel) => {
    ...
    updateChannel: async(function*(channelname) {
        ...
        let [data, info] = yield all([
            this._qs.queueRequest(baseURL+'status/'+channelname, headers, requeue),
            this._qs.queueRequest(infoURL+infoArgs, headers, requeue)
        ]);
    ...
    updateChannels: async(function*(channels) {
        ...
        let [data, info] = yield all([
            this._qs.queueRequest(baseURL+"all", headers, requeue),
            this._qs.queueRequest(infoURL+infoArgs, headers, requeue)
        ]);
