...
var { all, defer, resolve, reject } = require("sdk/core/promise");
...
const MLG = {
    ...
    getUserFavorites: function(username) {
        return reject("MLG.getUserFavorites is not supported.");
    },
    ...
    updateRequest: function(channels, callback) {
        qs.queueUpdateRequest([baseURL+"all"], headers, qs.HIGH_PRIORITY, requeue, async(function*(data) {
            let info = yield qs.queueRequest(infoURL+infoArgs, headers, requeue);
            ...
            if(data.json && data.json.status_code == 200 && info.json && info.json.status_code == 200) {
                ...
                return all(chans.map(function(status) {
