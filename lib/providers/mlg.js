...
const MLG = {
    ...
    getChannelDetails: function(channelname, callback) {
        qs.queueRequest(infoURL+infoArgs, headers, requeue).then(function(data) {
            if(data.status == 200 && data.json.status_code == 200) {
                ...
                getChannelFromJSON(cho).then(callback);
            }
        });
    },
    ...
    updateChannel: function(channelname, callback) {
        ...
        return all([
            ...
            if(data.json && data.json.status_code == 200 && info.json && info.json.status_code == 200) {
                return getChannelFromJSON(info.json.data.items.find((ch) => ch.id == data.json.data.channel_id))
                .then((channel) => {
    ...
    updateChannels: function(channels, callback) {
        ...
        return all([
            ...
            if(data.json && data.json.status_code == 200 && info.json && info.json.status_code == 200) {
                return all(data.json.data.items.filter((status) => {
                    return channels.some((channel) => status.stream_name == channel.login);
                }).map(function(status) {
                    return getChannelFromJSON(info.json.data.items.find(function(ch) { return ch.id == status.channel_id; }))
                    .then((channel) => {
                ...
                })).then((chs) => { callback(chs); return chs; });
            }
        });
    }
};
