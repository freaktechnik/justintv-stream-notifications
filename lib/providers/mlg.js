    ...
    if(!game) {
        qs.queueRequest(gameURL, headers, requeue, function(data) {
            if(data.json && data.json.data.items && data.json.data.items.length) {
                ...
                p.resolve(data.json.data.items.find((g) => g.id == id).name);
            }
            else {
                p.reject(data.json.errors);
            }
        });
    }
    else {
...
const MLG = {
    ...
    getChannelDetails: function(channelname, callback) {
        qs.queueRequest(infoURL+infoArgs, headers, requeue, function(data) {
            if(data.status == 200 && data.json.status_code == 200) {
                getChannelFromJSON(data.json.data.items.find(function(ch) { return ch.slug.toLowerCase() == channelname.toLowerCase(); }))
                    .then(callback);
            }
        });
    },
    ...
    updateChannel: function(channelname, callback) {
        ...
        qs.queueRequest(baseURL+'status/'+channelname, headers, requeue, function(data) {
            qs.queueRequest(infoURL+infoArgs, headers, requeue, function(info) {
                ...
                if(data.json && data.json.status_code == 200 && info.json && info.json.status_code == 200) {
                    getChannelFromJSON(info.json.data.items.find((ch) => ch.id == data.json.data.channel_id))
                    .then((channel) => {
