...
const Dummy = {
    ...
    getUserFavorites: function(username, userCallback, channelsCallback) {
        throw new Error(this.name+".getUserFavorites is not supported.");
    },
    getChannelDetails: function(channelname) {
        return qs.queueRequest(baseURL+"channels/"+channelname+".json", headers, requeue).then(function(response) {
