...
const getHitboxChannelDetails = function(channelname) {
    return qs.queueRequest(baseURL+'/media/live/'+channelname, headers, requeue).then(function(data) {
...
const Hitbox = {
    ...
    getUserFavorites: function(username) {
        //TODO
        return qs.queueRequest(baseURL+''+username, headers, requeue).then(function(data) {
            var channels = [];
            data.follows.forEach(function(channel) {
                channels.push(Object.assign(new Channel(), channel));
            });
            return [ Object.assign(new User(), data.user), channels ];
        });
    },
