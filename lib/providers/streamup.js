...
const getChannelFromJSON = (json) => {
    ...
    chan.uname = json.name !== "" ? json.name : json.username;
...
const Streamup = newClass({
    ...
    getUserFavorites: async(function*(username) {
        ...
        if(userData.json && "user" in userData.json) {
            ...
            let usrJson = userData.json.user;
            let user = new User();
            user.login = usrJson.username;
            user.uname = usrJson.name !== "" ? usrJson.name : usrJson.username;
            user.image = {
                64: usrJson.avatar.small,
                200: usrJson.avatar.medium,
                600: usrJson.avatar.large
            };
