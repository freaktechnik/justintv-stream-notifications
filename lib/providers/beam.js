...
const Beam = newClass({
    ...
    setup: function(type) {
        GenericProvider.prototype.call(this, type);
        
        this._ws = wss.getHelper({
            url: baseURL + "live",
            addItemCommand: (itemSpec) => {
                if(itemSpec.type == "channel")
                    return "channel:" + itemSpec.id + ":update";
                else
                    return "user:" + itemSpec.id + ":followed"
            },
            removeItemCommand: (itemSpec) => {
                if(itemSpec.type == "channel")
                    return "channel:" + itemSpec.id + ":update";
                else
                    return "user:" + itemSpec.id + ":followed";
