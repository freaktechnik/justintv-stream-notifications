...
const Beam = newClass({
    ...
    getFeaturedChannels() {
        return this._qs.queueRequest(baseURL+"channels?limit=8&page=0&order=online%3Adesc%2CviewersCurrent%3Adesc%2CviewersTotal%3Adesc&where=suspended.eq.0").then((data) => {
            if(data.json && data.json.length) {
                return Promise.all(data.json.map((chan) => {
    ...
    search(query) {
        return this._qs.queueRequest(baseURL+"channels?where=token.eq."+query).then((data) => {
            if(data.json && data.json.length) {
                return Promise.all(data.json.map((chan) => {
                    let ch = getChannelFromJSON(chan);
                    return this._qs.queueRequest(baseURL+"users/"+chan.user.id).then((resp) => {
