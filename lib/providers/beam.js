...
function getChannelFromJSON(jsonChannel) {
    ...
    ret.thumbnail  = "https://thumbs.beam.pro/" + jsonChannel.id + ".big.jpg";
...
function getUserIdFromUsername(username, callback) {
    qs.queueRequest(baseURL+"users/search?query="+username, headers, requeue).then(function(response) {
...
const Beam = {
    ...
    getUserFavorites: function(username, userCallback, channelsCallback) {
        getUserIdFromUsername(username, function(userid) {
            qs.queueRequest(baseURL+"users/"+userid, headers, requeue).then(function(user) {
                if(user.json) {
                    var ch = new User();
    ...
    getChannelDetails: function(channelname, callback) {
        qs.queueRequest(baseURL+"channels/"+channelname, headers, requeue).then(function(response) {
            if(response.json) {
                var channel = getChannelFromJSON(response.json);
                qs.queueRequest(baseURL+"users/"+response.json.user.id, headers, requeue).then(function(resp) {
                    if(resp.json)
