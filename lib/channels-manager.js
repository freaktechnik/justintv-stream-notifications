 ...
 */
 //TODO removechannel update request problem (better, but still troublesome in edge cases)
...
ChannelsManager.prototype.refreshUserFavorites = function(userId, type) {
    if(type) {
        ...
        list.getUser(function(user) {
            providers[user.type].getUserFavorites(user.login, list.setUser.bind(list), function(channels) {
                channels.forEach(function(channel) {
                    list.setChannel(channel);
                });
            });
