...
/***/ function(module, exports, __webpack_require__) {
	...
	const filters = [{
	    ...
	    subtarget: "small"
	      ...
	      listener = function () {
	    (0, _filter.filter)(document.getElementById("searchField").value, document.querySelector(".selectableItemsList:not([hidden])"), filters);
	};
	window.addEventListener("load", () => {
	    ...
	    document.getElementById("users").addEventListener("itemadded", () => {
	        if (!document.getElementById("users").hidden) listener();
	    });
	});
	...
	function resetDialogForms() {
	    ...
	    (0, _utils.hide)(popup.querySelector("#loadingWrapper"));
	    hideError();
	}
	...
	if (document.querySelector(".tabbed a.current") && document.querySelector(".tabbed a.current").dataset.tab == 1) {
	    (0, _utils.hide)(document.querySelector("#autoAdd").parentNode);
	    checkChannel();
	} else {
	    checkUser();
	}
	...
	document.querySelector("main.tabbed").addEventListener("tabchanged", evt => {
	    if (evt.detail == 1) {
	        ...
	        (0, _utils.show)(document.querySelector("#autoAdd").parentNode);
	...
	popup.querySelector("form").addEventListener("submit", evt => {
	    ...
	    hideError();
	    (0, _utils.show)(popup.querySelector("#loadingWrapper"));
	...
	port.onMessage.addListener(message => {
	    if (message.target == "secondary") {
	        (0, _utils.show)(document.querySelector("#secondary-manager"));
